...
public:
    ...
    void run(vector<Point> &all_points)
    {
        ...
        while (true)
        {
            //Monitoring iteration in process
            cout << "Iter - " << iter << "/" << iters << endl;
            bool done = true;

            // Open Mp repartition of processing
            ...
            if (done || iter >= iters)
            {
                cout << "Result Clustering " << iter << endl
                     << endl;
                break;
            }
            iter++;
        ...
        ofstream outfile;
        outfile.open(output_dir + "/Dump_result" + to_string(K) + ".txt");
        if (outfile.is_open())
        {
            for (int i = 0; i < K; i++)
            {
